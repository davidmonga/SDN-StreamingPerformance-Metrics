...
class VideoStreamingClient:
    @staticmethod
    def start_streaming(server, client, port, nginx_config, video_url, nom_interface_attendu, protocole, codec, perturbation, nombre_hotes, bw, jitter):
        ...
        # Appliquer la commande tc
        client.cmd(f'tc qdisc add dev {nom_interface_attendu} root netem delay {bw}ms {jitter}ms distribution normal')               
...
class Main:
    ...
    def main():
        ...
        jitters = video_input.get_jitters_numbers()
        ...
        for number in HOST_NUMBERS:
            ...
            for i, bw in enumerate(PERTURBATION_NUMBERS):
                jitter = jitters[i]                        
                print(f"\n Testing with delay: {bw} ms and jitter {jitter} ms \n")
                net.configLinkStatus('s1', 's2', 'down')
                net.addLink('s1', 's2', cls=TCLink, delay=f'{bw}ms', jitter=f'{jitter}ms', use_htb=True)
                ...
                for i in range(len(servers_experiment)):
                    t = threading.Thread(target=VideoStreamingClient.start_streaming, args=(net.get(servers_experiment[i]), net.get(clients_experiment[i]), ports_experiment[i], nginx_configs_experiment[i], video_urls_experiment[i], nom_interface_attendus_experiment[i], protocole, codec, perturbation, nombre_use_hotes, bw, jitter))
