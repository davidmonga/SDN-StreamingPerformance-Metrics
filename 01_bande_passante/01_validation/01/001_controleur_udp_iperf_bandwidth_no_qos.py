...
class QoS:
    ...
    def update_qos(self, bw, port1, port2):
        # Met à jour la configuration QoS sans appliquer de règles
            ...
            while True:
                for flow in self.qos_flows:
                    ...
                    if usage > flow['bw']:
                        ...
                        self.logger.info(f"Alerte QoS : Le(s) port(s) {flow['ports']} ont dépassé {flow['bw']}kbps. Excès : {exceeded}kbps")
                    else:
