...
class VideoInput:
    def __init__(self):
        self._servers = ['h1', 'h3', 'h5', 'h7', 'h9', 'h11']
        self._clients = ['h2', 'h4', 'h6', 'h8', 'h10', 'h12']
        self._HOST_NUMBERS = [12]
        #self._PERTURBATION_NUMBERS = [16, 15, 12, 10, 8, 6, 4, 3]
        self._PERTURBATION_NUMBERS = [24, 32, 48, 64, 80, 96]
        self._BURST = [0.3, 0.4, 0.6, 0.8,  1, 1.2]
        self._LATENCY = [100, 100, 100, 100, 100, 100]
        self._ports = [5000, 6000, 7000, 8000, 9000, 10000]
        self._nginx_configs = [
        ...
        self._video_urls = [
            "http://10.1.1.1:5000/h264/hls/1080/240/bbb_h264_slave_240_4.m3u8", "http://10.1.1.3:6000/h264/hls/1080/240/bbb_h264_slave_240_4.m3u8",
            "http://10.1.1.5:7000/h264/hls/1080/240/bbb_h264_slave_240_4.m3u8", "http://10.1.1.7:8000/h264/hls/1080/240/bbb_h264_slave_240_4.m3u8",
            "http://10.1.1.9:9000/h264/hls/1080/240/bbb_h264_slave_240_4.m3u8", "http://10.1.1.11:10000/h264/hls/1080/240/bbb_h264_slave_240_4.m3u8"
        ]
        self._nom_interface_attendus = ["h2-eth0", "h4-eth0", "h6-eth0", "h8-eth0", "h10-eth0", "h12-eth0"]
        self._protocole = "hls"
        self._codec = "h264"
        self._perturbation = "bande_passante"
        self._nombre_hotes = 12
        self._profile = 240
...
class VideoStreamingClient:
    @staticmethod
    def start_streaming(server, client, port, nginx_config, video_url, nom_interface_attendu, protocole, codec, perturbation, nombre_hotes, bw, burst, latency):
        ...
        # Attendre que la commande ffplay se termine
        # client.cmd('wait')
        time.sleep(30)
...
class Main:
    ...
    def main():
        ...
        bursts = video_input.get_burst()
        latencys = video_input.get_latency()
        j = 0
        for number in HOST_NUMBERS:
            ...
            for j, bw in enumerate(PERTURBATION_NUMBERS):
                ...
                for i in range(len(servers_experiment)):
                    t = threading.Thread(target=VideoStreamingClient.start_streaming, args=(net.get(servers_experiment[i]), net.get(clients_experiment[i]), ports_experiment[i], nginx_configs_experiment[i], video_urls_experiment[i], nom_interface_attendus_experiment[i], protocole, codec, perturbation, nombre_use_hotes, bw, burst, latency))
